...
# Generate flows dynamically - one flow per prefix
...
def run_all_flows(max_workers: int = 50):
    """Run all flows concurrently using threads (multiple runs per flow)."""
    # Calculate total number of runs
    total_runs = sum(FLOW_PATTERNS.values())

    print(f"Running {len(flows)} flows ({total_runs} total runs) concurrently with {max_workers} workers...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"Generate and run/deploy {len(flows)} Prefect flows with {sum(FLOW_PATTERNS.values())} total deployments"
    )
    parser.add_argument(
        "--deploy",
        action="store_true",
    ...
    total_runs_or_deployments = sum(FLOW_PATTERNS.values())
    print(f"Generated {len(flows)} flows with {total_runs_or_deployments} total runs/deployments:")
    for prefix, count in FLOW_PATTERNS.items():
